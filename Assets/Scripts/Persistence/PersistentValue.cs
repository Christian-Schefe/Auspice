using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class PersistentValue<T>
{
    private readonly string key;
    private readonly T defaultValue;
    private readonly PersistenceMode mode;

    private SaveState saveState;

    public PersistentValue(string key, PersistenceMode mode, T defaultValue)
    {
        this.key = key;
        this.defaultValue = defaultValue;
        this.mode = mode;
    }

    private void FindSaveState()
    {
        if (!SaveState.TryGetInstance(mode, out saveState))
        {
            throw new System.Exception($"SaveState with mode {mode} not found");
        }
    }

    public void AddListener(System.Action<T> action)
    {
        if (saveState == null) FindSaveState();
        saveState.AddListener(key, action);
    }

    public void RemoveListener(System.Action<T> action)
    {
        if (saveState == null) FindSaveState();
        saveState.RemoveListener(key, action);
    }

    public T Get()
    {
        if (saveState == null) FindSaveState();
        if (!saveState.TryGet(key, out T value))
        {
            Set(defaultValue);
            return defaultValue;
        }
        return value;
    }

    public ref T GetRef()
    {
        if (saveState == null) FindSaveState();
        if (!saveState.TryGetBox(key, out SaveState.Box<T> box))
        {
            Debug.Log("PersistentValue.GetRef: " + key + " not found");
            Set(defaultValue);
            saveState.TryGetBox(key, out box);
        }
        return ref box.value;
    }

    public void Set(T value)
    {
        if (saveState == null) FindSaveState();
        saveState.Set(key, value);
    }

    public void MarkDirty()
    {
        if (saveState == null) FindSaveState();
        saveState.OnUpdate(key);
    }
}

public class ReadonlyPersistentValue<T>
{
    private readonly string key;
    private readonly PersistenceMode mode;

    private SaveState saveState;

    public ReadonlyPersistentValue(string key, PersistenceMode mode)
    {
        this.key = key;
        this.mode = mode;
    }

    private void FindSaveState()
    {
        if (!SaveState.TryGetInstance(mode, out saveState))
        {
            throw new System.Exception($"SaveState with mode {mode} not found");
        }
    }

    public void AddListener(System.Action<T> action)
    {
        if (saveState == null) FindSaveState();
        saveState.AddListener(key, action);
    }

    public void RemoveListener(System.Action<T> action)
    {
        if (saveState == null) FindSaveState();
        saveState.RemoveListener(key, action);
    }

    public void MarkDirty()
    {
        if (saveState == null) FindSaveState();
        saveState.OnUpdate(key);
    }

    public bool TryGet(out T value)
    {
        if (saveState == null) FindSaveState();
        return saveState.TryGet(key, out value);
    }

    public T GetDefault(out T value, T defaultValue = default)
    {
        if (saveState == null) FindSaveState();
        return saveState.TryGet(key, out value) ? value : defaultValue;
    }

    public T Get()
    {
        if (saveState == null) FindSaveState();
        if (!saveState.TryGet(key, out T value))
        {
            throw new System.Exception($"ReadonlyPersistentValue.Get: {key} not found");
        }
        return value;
    }
}
